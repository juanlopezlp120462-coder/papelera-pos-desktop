...
$SUPABASE_UPDATE_FILE = Join-Path $ROOT "_supabase_update_temp.py"
...
if not SUPABASE_SERVICE_KEY:
    raise Exception("Falta SUPABASE_SERVICE_KEY en .env")
...
# ============================================================
# PUBLICAR VERSION MEDIANTE RPC SEGURA
# ============================================================

print("Publicando nueva version...")
...
if not resultado.data:
    raise Exception(
        "Supabase no devolvio resultado al publicar la version."
    )
...
try {
    ...
    Set-Content `
        -LiteralPath $SUPABASE_UPDATE_FILE `
        -Value $SUPABASE_UPDATE `
        -Encoding UTF8

    Write-Host "Ejecutando publicacion en Supabase..." -ForegroundColor DarkGray
    Write-Host ""

    $SUPABASE_RESULT = python $SUPABASE_UPDATE_FILE
    ...
    Write-Host "Supabase NO fue actualizado." -ForegroundColor Yellow
    Write-Host ""
    ...
    if (Test-Path -LiteralPath $SUPABASE_UPDATE_FILE) {
